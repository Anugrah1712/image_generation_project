...
def refine_prompt(original_prompt):
    print("🪄 Refining prompt using Gemini...")
    system_instruction = (
    ...
    if not refined:
        print("⚠️ Prompt refinement failed. Using original prompt.")
...
if not os.path.exists(inswapper_path):
    print("⬇️ Downloading InsightFace inswapper weights...")
...
if not os.path.exists(gfpgang_path):
    print("⬇️ Downloading GFPGAN weights...")
...
try:
    ...
    ctx_id = 0
    print("✅ GPU detected. Using GPU for InsightFace.")
except Exception:
    ctx_id = -1
    print("⚠️ GPU not detected. Using CPU for InsightFace.")
...
def insightface_swap(source_path, target_path, output_path):
    print(f"🔹 Starting face swap: source={source_path}, target={target_path}")
    ...
    if not source_faces or not target_faces:
        print("⚠️ No face detected in one of the images. Trying reversed input...")
        source_faces, target_faces = target_faces, source_faces
        if not source_faces or not target_faces:
            return None, "❌ No face detected in either image (even after retry)."
    ...
    try:
        ...
        return None, f"❌ Face swap failed: {e}"
...
def process_images(prompt, img1_path=None, img2_path=None, face_swap=False):
    print("🚀 Starting full image processing pipeline...")

    if face_swap:
        print("🎭 Face Swap mode enabled — skipping Gemini image generation.")

        if not img1_path or not img2_path:
            raise ValueError("❌ Face swap mode requires two images!")
        ...
        print("🔹 Enhancing swapped result using GFPGAN (Real-ESRGAN removed)...")
        ...
        print(f"✅ Final image (GFPGAN only) saved: {final_path}")

        return final_path

    else:
        print("🪄 Using Gemini to generate/edit image...")
